...
namespace smplx::io {
struct NpyArray {
    NpyArray(const std::vector<long long> &_shape, size_t _word_size,
             bool _fortran_order)
...
void npy_save(const std::string &fname, const T *data,
    ...
    FILE *fp = NULL;
    std::vector<size_t> true_data_shape; // if appending, the shape of
                                         // existing + new data
    ...
    if (fp) {
        // file exists. we need to append to it. read the
        // header, modify the array size
        size_t word_size;
        bool fortran_order;
        ...
        if (true_data_shape.size() != shape.size()) {
            std::cout << "libnpy error: npy_save attempting to "
                         "append misdimensioned data to "
                      << fname << "\n";
        ...
        for (size_t i = 1; i < shape.size(); i++) {
            if (shape[i] != true_data_shape[i]) {
                std::cout << "libnpy error: npy_save attempting "
                             "to append misshaped data to "
                          << fname << "\n";
...
void npz_save(const std::string &zipname, std::string fname, const T *data,
    ...
    if (fp) {
        // zip file exists. we need to add a new npy file to
        // it. first read the footer. this gives us the
        // offset and size of the global header then read
        // and store the global header. below, we will write
        // the the new data at the start of the global
        // header then append the global header and footer
        // below it
    ...
    global_header += (uint16_t)0; // internal file
                                  // attributes
    global_header += (uint32_t)0; // external file
                                  // attributes
    ...
    footer += (uint32_t)(global_header_offset + nbytes +
                         local_header.size()); // offset of start of
                                               // global headers, since
                                               // global header now starts
                                               // after newly written
                                               // array
...
std::vector<char> create_npy_header(const std::vector<size_t> &shape) {
    ...
    if (shape.size() == 1)
        dict += ",";
    dict += "), }";
    // pad with spaces so that preamble+dict is modulo 16
    // bytes. preamble is 10 bytes. dict needs to end with
    // \n
